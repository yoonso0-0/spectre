...
/// `initial_guess` mutator.  If `ExactSolution` is returned, the
/// implicit solve is skipped.
enum class GuessResult { InitialGuess, ExactSolution };

/// Mutator for the `initial_guess` of an implicit sector that uses
/// the result of the explicit step without modification.
struct GuessExplicitResult {
  using return_tags = tmpl::list<>;
  using argument_tags = tmpl::list<>;
  template <typename ImplicitVars>
  static GuessResult apply(const ImplicitVars& /*inhomogeneous_terms*/,
                           const double /*implicit_weight*/) {
    return GuessResult::InitialGuess;
  }
};
