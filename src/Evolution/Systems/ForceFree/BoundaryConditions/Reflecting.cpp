...
Reflecting::Reflecting(CkMigrateMessage* const msg) : BoundaryCondition(msg) {}

std::unique_ptr<domain::BoundaryConditions::BoundaryCondition>
Reflecting::get_clone() const {
  return std::make_unique<Reflecting>(*this);
}

void Reflecting::pup(PUP::er& p) { BoundaryCondition::pup(p); }

// NOLINTNEXTLINE
PUP::able::PUP_ID Reflecting::my_PUP_ID = 0;

std::optional<std::string> Reflecting::dg_ghost(
  ...
  Variables<tmpl::list<TildeJ, gr::Tags::SpatialMetric<DataVector, 3>,
  ...
  // Kill all the electric current from boundary.
  auto& exterior_tilde_j = get<TildeJ>(temp_buffer);
