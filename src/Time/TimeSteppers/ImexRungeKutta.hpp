...
class ImexRungeKutta : public virtual RungeKutta,
 ...
 public:
  ...
  struct ImplicitButcherTableau {
     ...
     * with an empty first row.  As with the explicit tableau, the
     * initial blank row should be omitted.  The final row must be the
     * same as the result coefficients in the explicit tableau, but
     * for consistency it must be included here.
     */
