...
SPECTRE_TEST_CASE("Unit.Evolution.Systems.ForceFree.Subcell.SwapGrTags",
                  "[Unit][Evolution]") {
  const Mesh<3> dg_mesh{5, Spectral::Basis::Legendre,
                        Spectral::Quadrature::GaussLobatto};
  const Mesh<3> subcell_mesh = evolution::dg::subcell::fd::mesh(dg_mesh);

  const ForceFree::System::spacetime_variables_tag::type active_gr_vars(
      dg_mesh.number_of_grid_points());
  const evolution::dg::subcell::Tags::Inactive<
      typename ForceFree::System::spacetime_variables_tag>::type
      inactive_gr_vars(subcell_mesh.number_of_grid_points());

  auto box = db::create<db::AddSimpleTags<
      ForceFree::System::spacetime_variables_tag,
  ...
  for (const auto active_grid : {evolution::dg::subcell::ActiveGrid::Dg,
    ...
    db::mutate_apply<ForceFree::subcell::SwapGrTags>(make_not_null(&box));

    if (active_grid == evolution::dg::subcell::ActiveGrid::Dg) {
      CHECK(db::get<ForceFree::System::spacetime_variables_tag>(box)
                .number_of_grid_points() == dg_mesh.number_of_grid_points());
      CHECK(db::get<evolution::dg::subcell::Tags::Inactive<
                ForceFree::System::spacetime_variables_tag>>(box)
