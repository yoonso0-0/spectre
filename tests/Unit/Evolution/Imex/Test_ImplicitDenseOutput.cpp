...
SPECTRE_TEST_CASE("Unit.Evolution.Imex.ImplicitDenseOutput",
  ...
  auto box = db::create<db::AddSimpleTags<
      System::variables_tag, imex::Tags::ImplicitHistory<Sector<Var1>>,
      imex::Tags::ImplicitHistory<Sector<Var2>>,
      Tags::TimeStepper<TimeSteppers::Heun2>, Tags::Time>>(
      initial_vars, std::move(history1), std::move(history2),
      std::make_unique<TimeSteppers::Heun2>(), test_time);
  db::mutate_apply<imex::ImplicitDenseOutput<System>>(make_not_null(&box));

  auto expected = initial_vars;
  db::get<Tags::TimeStepper<>>(box).dense_update_u(make_not_null(&expected),
                                                   full_history, test_time);
  CHECK(db::get<Var1>(box) == get<Var1>(expected));
  CHECK(db::get<Var2>(box) == get<Var2>(expected));
}
