...
void test(const gsl::not_null<std::mt19937*> gen) {
  ...
  // - Use Kerr-Schild or TOV solution as background metric, depending on the
  //    type (compile or runtime) of initial data to do the test.
  // - Create a box with tags necessary for running `BackgroundGrVars` action.
  // - Test the action for initial time.
  // - Mutate inertial coordinates and time to a random later moment, and test
  //    the action again.
  ...
  // test that the BackgroundGrVars action properly initializes (allocate +
  ...
  // Execute the action for initialization, and check that it has put correct
  // values of GR variables in the box.
  evolution::Actions::BackgroundGrVars<SystemForTest, false>::apply(
  ...
  evolution::Actions::BackgroundGrVars<SystemForTest, true>::apply(
      box, tuples::TaggedTuple<>{},
      Parallel::GlobalCache<MetavariablesForTest>{}, 0, tmpl::list<>{},
      std::unique_ptr<size_t>{}.get());
...
SPECTRE_TEST_CASE("Unit.Evolution.Actions.BackgroundGrVars",
                  "[Unit][Evolution]") {
  MAKE_GENERATOR(gen);
